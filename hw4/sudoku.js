...
var printBoard = function () {   
    for (let i = 0; i < 9; i++){ //first array element
        for (let j = 0; j < 9; j++){ //second array element
            document.getElementById(`r${i+1}${j+1}`).innerHTML = sol[i][j]; //sets the value of each cell to each respective value in the array
        }
    } 
};
...
var loadNewPuzzle = function() {
    ...
    //use this end point to get one puzzle
     request.open("GET", "https://sudoku-api.vercel.app/api/dosuku"); //opens the API
     request.onload=function() { //function to drill down into the JSON
        var data = JSON.parse(this.response); //stores the parsing of the JSON in a variable
        sol = data.newboard.grids[0].value; //assigns the value of the new puzzle to the array sol
        for (let i = 0; i < 9; i++){ //first array element
            for (let j = 0; j < 9; j++){ //second array element
                document.getElementById(`r${i+1}${j+1}`).innerHTML = sol[i][j]; //preps the new puzzle by setting each cell equal to the respective sol value
            }
        } 
     }
     
    request.send(); //sends the request to access the Sudoku API
};
