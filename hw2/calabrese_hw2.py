...
def middle_three(str):
    ...
    if len(str) < 3 or len(str) % 2 == 0: # checks to see if string length is even or smaller than 3
        return ""
    else:
        mid = len(str) // 2 #finds the middle index through floor division
        triple = f"{str[mid-1]}{str[mid]}{str[mid+1]}" #creates a string of the middle three characters
        return triple
...
def filter_long_words(words, length):
    ...
    """
    return [words[i] for i in range(0,len(words)) if len(words[i]) > length]
...
def merge_dictionaries(dict1, dict2):
    ...
    """
    md = {}
    for key,value in dict1.items(): #replicates dict1 into new dictionary
        md[key] = value
    for key,value in dict2.items(): #appends dict2 while overriding existing values for keys from dict2 into the new dictionary
        md[key] = value
    return md
...
def character_frequency(strings):
    ...
    for i in range(0,len(strings)):
        for j in range (0,len(strings[i])):
            if strings[i][j] not in cf.keys(): #starts the counter for letters showing up for the first time
                cf[strings[i][j]] = 1
            else:
                cf[strings[i][j]] += 1 #increments values for letters showing up more than once
    return cf
...
def group_by_length(strings):
    ...
    gl = {}
    for i in range(0,len(strings)):
        if len(strings[i]) in gl.keys(): #append onto strings for each length number
            gl[len(strings[i])].append(strings[i])
        else:
            gl[len(strings[i])] = [strings[i]] #initializes new keys for new length numbers
    return gl
